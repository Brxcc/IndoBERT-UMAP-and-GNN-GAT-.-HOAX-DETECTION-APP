...
const CM_DEF = {
  ...
  def: "A table that visualizes model performance by showing how many predictions were correct (TP, TN) and how many were incorrect (FP, FN) for each class.",
  formula: "Accuracy = (TP+TN)/(TP+TN+FP+FN)  |  Precision = TP/(TP+FP)  |  Recall = TP/(TP+FN)",
  interp: "TP (True Positive): Model predicted Hoax and it was actually Hoax. TN (True Negative): Model predicted Non-Hoax and it was actually Non-Hoax. FP (False Positive): Model incorrectly predicted Hoax. FN (False Negative): Model missed a real Hoax.",
  critical: "High TP and TN with low FP and FN indicates a well-performing model. A high FN is especially dangerous in hoax detection as it means real hoaxes are passing undetected.",
};
...
function ConfusionMatrixPanel({ compResult, matchTest }) {
  const [infoOpen, setInfoOpen] = useState(false);
  if (!compResult) return null;

  // --- Try to get EXACT CM from best epoch log (new models post-update) ---
  let TP = null, TN = null, FP = null, FN = null;
  let isEstimated = false;

  const epochLogs = compResult.epoch_logs || [];
  const bestLog = epochLogs.find(l => l.is_best) || epochLogs[epochLogs.length - 1];

  if (bestLog && bestLog.cm_tp != null) {
    TP = bestLog.cm_tp;
    TN = bestLog.cm_tn;
    FP = bestLog.cm_fp;
    FN = bestLog.cm_fn;
  } else {
    // --- Fallback: Estimate CM from stored aggregate metrics ---
    isEstimated = true;
    // Use matchTest metrics if available, otherwise use training result metrics
    const src = matchTest || compResult;
    const acc = src.accuracy ?? compResult.accuracy ?? 0;
    const prec = src.precision ?? compResult.precision ?? 0;
    const rec = src.recall ?? compResult.recall ?? 0;
    const n = matchTest?.total_rows ?? 100; // default n if unknown

    // Estimate total positives and negatives (assume ~50/50 split)
  ...
  const cells = [
    { label: "True Negative",  abbr: "TN", val: TN, color: "bg-green-50  border-green-200  text-green-800",  badge: "bg-green-100  text-green-700",  desc: "Correctly predicted Non-Hoax" },
    { label: "False Positive", abbr: "FP", val: FP, color: "bg-red-50    border-red-200    text-red-800",    badge: "bg-red-100    text-red-700",    desc: "Predicted Hoax, actually Non-Hoax" },
    { label: "False Negative", abbr: "FN", val: FN, color: "bg-amber-50  border-amber-200  text-amber-800",  badge: "bg-amber-100  text-amber-700",  desc: "Predicted Non-Hoax, actually Hoax" },
    { label: "True Positive",  abbr: "TP", val: TP, color: "bg-indigo-50 border-indigo-200 text-indigo-800", badge: "bg-indigo-100 text-indigo-700", desc: "Correctly predicted Hoax" },
  ];
    ...
    <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
      ...
      <div className="flex items-center justify-between mb-1">
        <h4 className="font-bold text-slate-700 flex items-center gap-2">
          Confusion Matrix
          {isEstimated && (
            <span className="text-[10px] bg-amber-100 text-amber-700 font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">Estimated</span>
          )}
          <button type="button" onClick={() => setInfoOpen(true)} className="text-slate-300 hover:text-blue-500 transition">
            <Info className="w-3.5 h-3.5" />
          </button>
        </h4>
      </div>
      <p className="text-xs text-slate-400 mb-6">
        {isEstimated
          ? "Values estimated from aggregate metrics (Accuracy, Precision, Recall). Retrain a model for exact data."
          : "Actual data from the best epoch during training."}
      </p>

      <div className="flex flex-col lg:flex-row gap-8 items-start justify-center">
        {/* Matrix Grid */}
        <div className="flex-shrink-0">
          <div className="text-center mb-2 text-xs font-bold text-slate-500 uppercase tracking-wider">Predicted Class</div>
          <div className="flex">
            <div className="flex flex-col items-center justify-center mr-2">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider" style={{writingMode:'vertical-rl', transform:'rotate(180deg)'}}>Actual Class</span>
            </div>
            <div>
              <div className="flex mb-1">
                <div className="w-20"></div>
                <div className="flex-1 text-center text-xs font-bold text-slate-500 px-2">Non-Hoax</div>
                <div className="flex-1 text-center text-xs font-bold text-slate-500 px-2">Hoax</div>
              </div>
              <div>
                <div className="flex items-center mb-1">
                  <div className="w-20 text-right pr-2 text-xs font-bold text-slate-500">Non-Hoax</div>
                  <div className={`w-28 h-24 border-2 rounded-xl flex flex-col items-center justify-center mr-1 ${cells[0].color}`}>
                    <span className="text-3xl font-black">{TN}</span>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full mt-1 ${cells[0].badge}`}>TN</span>
                  </div>
                  <div className={`w-28 h-24 border-2 rounded-xl flex flex-col items-center justify-center ${cells[1].color}`}>
                    <span className="text-3xl font-black">{FP}</span>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full mt-1 ${cells[1].badge}`}>FP</span>
                  </div>
                </div>
                <div className="flex items-center">
                  <div className="w-20 text-right pr-2 text-xs font-bold text-slate-500">Hoax</div>
                  <div className={`w-28 h-24 border-2 rounded-xl flex flex-col items-center justify-center mr-1 ${cells[2].color}`}>
                    <span className="text-3xl font-black">{FN}</span>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full mt-1 ${cells[2].badge}`}>FN</span>
                  </div>
                  <div className={`w-28 h-24 border-2 rounded-xl flex flex-col items-center justify-center ${cells[3].color}`}>
                    <span className="text-3xl font-black">{TP}</span>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full mt-1 ${cells[3].badge}`}>TP</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex-1 space-y-3 min-w-0">
          {cells.map(c => (
            <div key={c.abbr} className={`flex items-center justify-between p-3 rounded-xl border ${c.color}`}>
              <div>
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full mr-2 ${c.badge}`}>{c.abbr}</span>
                <span className="text-xs font-semibold">{c.label}</span>
                <p className="text-[10px] text-slate-500 mt-0.5 ml-0">{c.desc}</p>
              </div>
              <span className="text-xl font-black">{c.val}</span>
            </div>
          ))}
          ...
          <div className="grid grid-cols-2 gap-2 pt-1">
            <div className="bg-white rounded-xl border border-slate-200 p-3 text-center">
              ...
              <p className="text-xl font-black text-green-600">{TP + TN}</p>
              <p className="text-[10px] text-slate-500">of {total} ({((TP+TN)/total*100).toFixed(1)}%)</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-3 text-center">
              <p className="text-[10px] text-slate-400 font-bold uppercase">Incorrect</p>
              <p className="text-xl font-black text-red-600">{FP + FN}</p>
              <p className="text-[10px] text-slate-500">of {total} ({((FP+FN)/total*100).toFixed(1)}%)</p>
            </div>
          </div>
        </div>
      </div>

      {infoOpen && <ConfusionMatrixInfoModal onClose={() => setInfoOpen(false)} />}
    </div>
  );
}
                    ...
                    {!compResult
                        ...
                        : (
                            <>
                                ...
                                {/* Confusion Matrix Panel */}
                                <ConfusionMatrixPanel compResult={compResult} matchTest={matchTest} />
                            </>
                        )
                    }
                </div>
